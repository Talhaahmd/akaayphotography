...
(function ($) {
	...
	// SCROLL TRANSITION FOR NARRATIVE
	if ('IntersectionObserver' in window) {
		...
		$('.scroll-transition-section').each(function () {
			narrativeObserver.observe(this);
		});
	}

	// WHATSAPP MOBILE SAFETY
	$(document).ready(function () {
		function ensureWhatsApp() {
			if ($(window).width() < 768) {
				$('.whatsapp-sticky').css({ 'display': 'flex', 'visibility': 'visible', 'opacity': '1' });
			}
		}
		ensureWhatsApp();
